...
CalculatorApp::CalculatorApp()
		  		  : BApplication("application/x-vnd.Be-calculator")
{
	Prefs prefs;
	...
	r.right -= (20-5);
	r.bottom -= -3;
	if (prefs.h < r.left) prefs.h = (int)r.left;
	if (prefs.h > r.right) prefs.h = (int)r.right;
	if (prefs.v < r.top) prefs.v = (int)r.top;
	if (prefs.v > r.bottom) prefs.v = (int)r.bottom;
